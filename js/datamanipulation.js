...
function fixData() {
	for (var i = 0; i < data.length; i++)
		data[i].matches.sort((a, b) => priority(b) - priority(a));
}
...
function setOnChange() {
	$('.filter-input').on('input', function (e) {
		var tableIndex = parseInt(e.currentTarget.id.split("-")[1]);
		filterTable(tableIndex);
	});
}

function createLimitFilter(filters, i) {
	var label = document.createElement("label");
	label.innerHTML = 'Minimum Data Used: <input class="filter-input" type="number" value="20" id="filter-' + i + '"/>';
	filters.insertBefore(label, filters.firstChild);
}

function filterTable(tableIndex) {
	var limit = getLimit(tableIndex);
	var table = $("#predictionTable-" + tableIndex).DataTable();

	$.fn.dataTable.ext.search[tableIndex] = getFilter(tableIndex, limit);
	table.draw();
}

function getLimit(tableIndex) {
	var limit = document.getElementById("filter-" + tableIndex).value;
	if (!limit)
		limit = 0;
	return limit;
}

function getFilter(index, limit) {
	return function (settings, data, dataIndex) {
		if (settings.nTable.id != "predictionTable-" + index)
			return true;
...
function printAcca() {
	var acca = getAcca(data.find(function (a) {
		return new Date(a.targetDate).getDate() == new Date().getDate();
	}).matches, false);
...
function getValue(match, text) {
	switch (true) {
		case text.includes("Home"):
			return match.FT[0];
		case text.includes("Away"):
			return match.FT[2];
		case text.includes("2.5"):
			return match.O[2];
		case text.includes("1.5"):
			return match.O[1];
		case text.includes("GG"):
			return match.GG;
	}
}
